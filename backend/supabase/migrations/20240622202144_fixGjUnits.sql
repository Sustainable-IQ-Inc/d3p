create or replace view "public"."project_energy_summary" as  WITH ranked_uploads AS (
         SELECT p.id AS project_id,
            ...
            GREATEST(COALESCE(u.updated_at, '1900-01-01 00:00:00+00'::timestamp with time zone), COALESCE(ud.updated_at, '1900-01-01 00:00:00+00'::timestamp with time zone), COALESCE(p.updated_at, '1900-01-01 00:00:00+00'::timestamp with time zone)) AS most_recent_updated_at,
                ...
                CASE
                    WHEN (ud.energy_units = 'gj'::text) THEN (((ud.total_energy)::numeric - (ud."total_Electricity")::numeric) * 0.947817)
                    ELSE ((ud.total_energy)::numeric - (ud."total_Electricity")::numeric)
                ...
                CASE
                    WHEN (ud.energy_units = 'gj'::text) THEN ((((ud."total_Electricity")::numeric - (ud."total_On-SiteRenewables")::numeric) + ((ud.total_energy)::numeric - (ud."total_Electricity")::numeric)) * 0.947817)
                    ELSE (((ud."total_Electricity")::numeric - (ud."total_On-SiteRenewables")::numeric) + ((ud.total_energy)::numeric - (ud."total_Electricity")::numeric))
