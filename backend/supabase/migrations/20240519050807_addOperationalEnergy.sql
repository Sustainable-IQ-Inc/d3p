drop view if exists "public"."project_energy_summary4";

drop view if exists "public"."project_energy_summary";
...
create or replace view "public"."project_energy_summary" as  WITH ranked_uploads AS (
         SELECT p.id AS project_id,
            ...
            (ud."total_Electricity")::numeric AS building_total_electricity,
            (ud.total_energy)::numeric AS building_total_energy,
            ((ud.total_energy)::numeric - (ud."total_Electricity")::numeric) AS building_total_fossil_fuels,
            (ud."total_On-SiteRenewables")::numeric AS building_onsite_renewables,
            ((ud."total_Electricity")::numeric - (ud."total_On-SiteRenewables")::numeric) AS building_net_electricity,
            (((ud."total_Electricity")::numeric - (ud."total_On-SiteRenewables")::numeric) + ((ud.total_energy)::numeric - (ud."total_Electricity")::numeric)) AS net_operational_energy_total
           FROM ((((((((projects p
         ...
         SELECT ranked_uploads.project_id,
            ...
            ranked_uploads.total_energy,
                CASE
                    WHEN (ranked_uploads.conditioned_area > (0)::numeric) THEN (((ranked_uploads.total_energy)::numeric / ranked_uploads.conditioned_area) * (1000)::numeric)
                    ELSE NULL::numeric
 ...
 SELECT COALESCE(b.project_id, d.project_id) AS project_id,
    ...
    (b.total_energy)::numeric AS total_energy_baseline,
    (d.total_energy)::numeric AS total_energy_design,
    ...
    ((1)::numeric - (d.total_energy_per_unit_area_design / b.total_energy_per_unit_area_baseline)) AS pct_operational_energy_savings
   FROM (baseline b
