-- Fix project_energy_summary view to order by updated_at instead of created_at
-- This ensures that when uploads are updated, the view shows the most recently updated upload
-- rather than just the most recently created one
-- 
-- Ordering logic:
-- 1. Uploads with updated_at (been modified) come first (0 < 1)
-- 2. Among those, most recently updated first (DESC)
-- 3. Among unmodified uploads, most recently created first (DESC)
