-- Add processing_error to project_energy_summary view
-- This allows the frontend to display error banners on project detail pages
...
create or replace view "public"."project_energy_summary" as  WITH ranked_uploads AS (
         SELECT p.id AS project_id,
            ...
            row_number() OVER (PARTITION BY p.id, ud.baseline_design ORDER BY CASE WHEN u.updated_at IS NOT NULL THEN 0 ELSE 1 END, u.updated_at DESC NULLS LAST, u.created_at DESC) AS rn,
            p.company_id,
                CASE
                    WHEN (
                        (
                          regexp_replace(ud."SolarDHW_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text) <> ''::text
                          AND (regexp_replace(ud."SolarDHW_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text))::numeric > 0
                        ) OR (
                          regexp_replace(ud."SolarPV_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text) <> ''::text
                          AND (regexp_replace(ud."SolarPV_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text))::numeric > 0
                        ) OR (
                          regexp_replace(ud."Wind_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text) <> ''::text
                          AND (regexp_replace(ud."Wind_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text))::numeric > 0
                        ) OR (
                          regexp_replace(ud."Other_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text) <> ''::text
                          AND (regexp_replace(ud."Other_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text))::numeric > 0
                        ) OR (
                          regexp_replace(ud."total_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text) <> ''::text
                          AND (regexp_replace(ud."total_On-SiteRenewables", '\\D'::text, ''::text, 'g'::text))::numeric > 0
                        )
                    ) THEN 'Yes'::text
                    ELSE 'No'::text
                END AS renewables,
            ...
            ud.city,
            ud.state,
            u.processing_error,  -- ADDED THIS FIELD
            u.reporting_year     -- ADDED THIS FIELD
           FROM (((((((((projects p
         ...
         SELECT ranked_uploads.project_id,
            ...
            ranked_uploads.processing_error,  -- ADDED THIS FIELD
            ranked_uploads.reporting_year     -- ADDED THIS FIELD
           FROM ranked_uploads
 ...
 SELECT COALESCE(b.project_id, d.project_id) AS project_id,
    ...
    COALESCE(b.processing_error, d.processing_error) AS processing_error,  -- ADDED THIS FIELD
    COALESCE(b.reporting_year, d.reporting_year) AS reporting_year         -- ADDED THIS FIELD
   FROM (baseline b
